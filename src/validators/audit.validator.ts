...
const facilityTypes = ['Manufacturing', 'Data Center', 'Corporate Office', 'Logistics Hub', 'Retail Store'] as const;
const scopeCategories = ['Scope 1 (Direct)', 'Scope 2 (Indirect Energy)', 'Scope 3 (Value Chain)'] as const;
const riskRatings = ['Low Carbon', 'Moderate Impact', 'High Emissions', 'Critical Failure'] as const;
...
export const createAuditSchema = z.object({
  title: z
    .string({ required_error: 'Audit title is required.' })
  ...
  facilityType: z.enum(facilityTypes, {
    errorMap: () => ({ message: `Facility type must be one of: ${facilityTypes.join(', ')}.` }),
  }),
  location: z
    ...
    .trim(),
  auditYear: z
    .number({ required_error: 'Audit year is required.' })
    .int('Audit year must be an integer.')
    .min(1900, 'Audit year must be 1900 or later.')
    .max(2100, 'Audit year cannot exceed 2100.'),
  scopeCategory: z.enum(scopeCategories, {
    errorMap: () => ({ message: `Scope category must be one of: ${scopeCategories.join(', ')}.` }),
  }),
  carbonScoreTons: z
    .number({ required_error: 'Carbon score (tons) is required.' })
    .min(0, 'Carbon score cannot be negative.'),
  energyUsageKwh: z
    .number({ required_error: 'Energy usage (Kwh) is required.' })
    .min(0, 'Energy usage cannot be negative.'),
  riskRating: z.enum(riskRatings, {
    errorMap: () => ({ message: `Risk rating must be one of: ${riskRatings.join(', ')}.` }),
  }),
  shortDescription: z
    ...
    .trim(),
  fullOverview: z
    .string({ required_error: 'Full overview is required.' })
...
export const queryAuditSchema = z.object({
  ...
  scopeCategory: z.enum(scopeCategories).optional(),
